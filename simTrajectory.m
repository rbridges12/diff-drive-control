...
function [tout, yout] = simTrajectory(x)
    ...
    interp_vel_l = @(t) interp1(linspace(0, tf, numel(x/2)), x(1:end/2-1), t);
    interp_vel_r = @(t) interp1(linspace(0, tf, numel(x/2)), x(end/2:end), t);
    ...
    odefun = @(t, y) [R/2*(interp_vel_r(t) + interp_vel_l(t)) * cos(y(3)); 
        ...
        R/L * (interp_velr_(t) - interp_vel_l(t))];
